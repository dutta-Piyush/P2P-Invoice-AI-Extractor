...
from services.request_service import IRequestService, get_request_service
...
def create_request(
    ...
    try:
        record = service.create(payload, db)
...
def get_request(
    ...
    try:
        record = service.get_by_id(request_id, db)
    except SQLAlchemyError as exc:
...
def update_status(
    ...
    try:
        record = service.update_status(request_id, payload.status, payload.note, db)
    except SQLAlchemyError as exc:
...
def download_document(
    ...
    try:
        path = service.get_source_pdf_path(request_id, db)
    except SQLAlchemyError as exc:
...
# Procurement records must never be deleted for GoBD compliance.
