...
import { classifyCommodityGroup, emptyForm, emptyLine, fmt, validate } from "../utils/procurementUtils";

export default function IntakeForm({ onSubmit, onNotice }) {
  ...
  const lineTotal = useMemo(
    () => form.order_lines.reduce((sum, line) => sum + (parseFloat(line.total_price) || 0), 0),
    [form.order_lines],
  );
  ...
  const updateLine = (id, field, value) => {
    setForm((prev) => ({
      ...
      order_lines: prev.order_lines.map((line) => {
        ...
        updated.total_price = parseFloat((unitPrice * amount).toFixed(2));
        return updated;
      }),
    }));
  };
  ...
  const removeLine = (id) => {
    setForm((prev) => ({
      ...prev,
      order_lines: prev.order_lines.length === 1 ? prev.order_lines : prev.order_lines.filter((line) => line.id !== id),
    }));
  };

  const addLine = () => {
    setForm((prev) => ({ ...prev, order_lines: [...prev.order_lines, emptyLine()] }));
  };
  ...
  const handleExtract = async (file) => {
    ...
    try {
      ...
      setForm((prev) => ({
        ...
        order_lines: data.order_lines?.map((line) => ({ ...line, id: Date.now() + Math.random() })) || prev.order_lines,
      }));
      ...
      onNotice(err.message || "Extraction failed. Fill details manually.");
    } finally {
      setExtracting(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onNotice(err.message || "Failed to submit request. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="stack">
      {extractWarnings.length > 0 && (
        <div className="extraction-warning">
          ...
          <button
            ...
            onClick={(e) => { e.stopPropagation(); setExtractWarnings([]); }}
      ...
      <section className="card">
        ...
        <div className="upload-zone" onClick={() => inputRef.current?.click()}>
          <input ref={inputRef} type="file" accept=".pdf,image/*" style={{ display: "none" }} onChange={(e) => handleExtract(e.target.files[0])} />
          <div className="upload-text">{extracting ? "Extracting..." : "Click to upload offer"}</div>
      ...
      <section className="card">
        ...
        <div className="form-grid">
          <div className="field">
            ...
            <input value={form.requestor_name} onChange={(e) => setField("requestor_name", e.target.value)} className={errors.requestor_name ? "error" : ""} />
            {errors.requestor_name && <small className="error-text">{errors.requestor_name}</small>}
          </div>

          <div className="field">
            <label>Department *</label>
            <input value={form.department} onChange={(e) => setField("department", e.target.value)} className={errors.department ? "error" : ""} />
            {errors.department && <small className="error-text">{errors.department}</small>}
          </div>

          <div className="field full">
            <label>Title / short description *</label>
            <input value={form.title} onChange={(e) => setField("title", e.target.value)} className={errors.title ? "error" : ""} />
            {errors.title && <small className="error-text">{errors.title}</small>}
          </div>

          <div className="field">
            <label>Vendor name *</label>
            <input value={form.vendor_name} onChange={(e) => setField("vendor_name", e.target.value)} className={errors.vendor_name ? "error" : ""} />
            {errors.vendor_name && <small className="error-text">{errors.vendor_name}</small>}
          </div>

          <div className="field">
            <label>VAT ID *</label>
            <input value={form.vat_id} onChange={(e) => setField("vat_id", e.target.value)} className={errors.vat_id ? "error" : ""} />
            {errors.vat_id && <small className="error-text">{errors.vat_id}</small>}
          </div>

          <div className="field full">
            <label>Commodity group (auto)</label>
            ...
            {form.order_lines.map((line, index) => (
              <div className="order-line" key={line.id}>
                <input
                  ...
                  onChange={(e) => updateLine(line.id, "position_description", e.target.value)}
                ...
                <input readOnly value={line.total_price ? fmt(line.total_price) : ""} />
                <button className="remove-btn" onClick={() => removeLine(line.id)}>
                  ×
                </button>
              </div>
            ))}
